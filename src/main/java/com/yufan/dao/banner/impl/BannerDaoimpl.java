...
public class BannerDaoimpl implements IBannerDao {
    ...
    public PageInfo loadDataPage(int currePage, BannerCondition bannerCondition) {
        ...
        sql.append(" select banner_id,banner_title,banner_name,CONCAT('").append(Constants.IMG_WEB_URL).append("',banner_img) as banner_img,banner_link,DATE_FORMAT(start_time,'%Y-%m-%d') as start_time,status, ");
        sql.append(" DATE_FORMAT(end_time,'%Y-%m-%d') as end_time,data_index,DATE_FORMAT(createtime,'%Y-%m-%d %T') as createtime ");
